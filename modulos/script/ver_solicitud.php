...
<?php
    switch ($table) {
        case 'rh_solicitudes':
            ...
            <input type="hidden" name="inp_dg" id="inp_dg" value="<?php echo($dg_aut = ($val_dg !== false) ? 0 : $dg_aut ); ?>">
            <?php
                if ($val_dg !== false) {
        ...
        case 'rh_vacaciones':
            ...
            if ($fecha_now >= $v_hire_one) {
                ...
                $v_mensaje .= '<br><div class="badge bg-info text-wrap" style="width: 26rem;">Este empleado tiene menos de un año laborando, se solicitara una autorizacion por direcci&oacute;n general.</div>';
                $dg_aut = 1;
            }

            if ($total_dias_sol > $vacation_rule) {
                //***Solicita mas dias de los disponibles
                $v_mensaje .= '<br><div class="badge bg-info text-wrap" style="width: 26rem;">No cuenta con dias disponibles del ciclo, bajo autorizacion de direccion general.</div>';
                $dg_aut = 1;
            }else{
                //***Solicitud correcta
                $dg_aut = 0;
            ...
            <?php
                ...
                if ($estatus_v == '1') {
                    echo '||<button type="button" class="btn btn-danger btn-sm" id="btn_act2" onclick="actions_solicitud(5, 0)">Rechazar</button>';
                }
            break;
    }
?>