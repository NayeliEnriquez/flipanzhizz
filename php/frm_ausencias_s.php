<?php
    ...
    $selected_rdo_inout_Val = $_POST['selected_rdo_inout_Val'];//***1-> Entrada || 2-> Salida
    $selected_rdo_asunto_Val = $_POST['selected_rdo_asunto_Val'];//***1-> Asunto trabajo || 2-> Asunto personal
    $selected_rdo_goce_Val = $_POST['selected_rdo_goce_Val'];//***1-> Con goce || 2-> Sin goce de sueldo
    ...
    $inp_hr_in_out = $_POST['inp_hr_in_out'];
    $inp_hr_in = '';
    $inp_hr_out = '';

    $inp_fhrepo = str_replace('T', ' ', $_POST['inp_fhrepo']);
    $inp_hrrepo = $_POST['inp_hrrepo'];
    ...
    if ($selected_rdo_inout_Val == '1') {
        $inout_Val_response = "Entrada";
        if (($selected_rdo_goce_Val == '1') && (!empty($inp_hr_in_out))) {
            $inp_hr_in = $inp_hr_in_out;
        }
    }else{
        $inout_Val_response = "Salida";
        if (($selected_rdo_goce_Val == '1') && (!empty($inp_hr_in_out))) {
            $inp_hr_out = $inp_hr_in_out;
        }
    }

    $query_a = "
        INSERT INTO dbo.rh_salida(
            id_empleado, tipo_ausencia, in_out,
            asunto, sueldo, f_permiso,
            h_permiso, observaciones, reposicion, 
            id_solicitante, id_depto, f_solicitud,
            estatus, h_in, h_out, 
            f_h_repo, hrs_repo)
        VALUES
            ('$inp_id_empleado', '$inout_Val_response', '$selected_rdo_inout_Val', 
            '$selected_rdo_asunto_Val', '$selected_rdo_goce_Val', '$inp_fecha_p',
            '$inp_hora_p', '$txt_obs', '$reponer_response',
            '$inp_id_empleado', '$inp_id_depto', '$fecha_hora_now',
            '0', '$inp_hr_in', '$inp_hr_out',
            '$inp_fhrepo', '$inp_hrrepo');
    ...
    if ($exe_a = sqlsrv_query($cnx, $query_a)) {
        ...
        //*****************************************
        if(!empty($email_boss)){
            $datos_bd = "db=rh_salida&idbd=$id_inserted&cod_boss=$emp_code_charge_boss";
            ...
            $msn ='
                <html lang="es">
                    ...
                    <body>
                        ...
                        <p>El empleado <strong>'.$full_name.' ('.$inp_id_empleado.')</strong> solicita un permiso de <strong>'.$inout_Val_response.'</strong>, el dia <strong>'.$inp_fecha_p.'</strong> a las <strong>'.$inp_hora_p.' horas</strong>.</p>
                        <p>Observaciones: <i>'.$txt_obs.'</i></p>
                        <p><strong>Acciones.</strong></p>
                        <!--<a href="http://aplicativo_rh.novag/action_mail/solicitud_s.php?action=1&db=rh_salida&idbd='.$id_inserted.'&cod_boss='.$emp_code_charge_boss.'" target="_blank">Aceptar</a>&nbsp;&nbsp;<a href="http://aplicativo_rh.novag/action_mail/solicitud_s.php?action=2&db=rh_salida&idbd='.$id_inserted.'&cod_boss='.$emp_code_charge_boss.'" target="_blank">Rechazar</a>&nbsp;&nbsp;<a href="http://192.168.1.12:88?user_mail='.$email_boss.'" target="_blank"">Modificar</a>-->
            ...
            try {
                ...
                $mail->isHTML(true);
                $mail->Subject = 'Solicitud de '.$inout_Val_response.' #'.$id_inserted;  // Asunto del mensaje
        ...
        //*****************************************
        $btn_imprime = '
            <button type="button" class="w-100 btn btn-primary btn-lg" title="Descargar formato" onclick="download_formatos(`rh_salida`, '.$id_inserted.', '.$inp_id_empleado.')">
                Descargar formato
